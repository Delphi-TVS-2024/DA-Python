...
class StartSending:
    ...
    def __init__(self, TOKEN_URL, TABLE_URL, FILE_DIR, table_list):
    ...
    def update_visualiser(self, table_name):
        try:
            ...
            self.json_creation()
            self.datalog('INFO', 'JSON', 'NEW VISUALISER JSON CREATED')
            json_file_data = json.loads(open(self.visualiser_json).read())
    ...
    def initialise_logs(self):
        ...
        try:
            ...
            with open(self.logfile_name, 'w', newline='') as file:
                ...
                write_log_csv.writerow([datetime.now(), 'INFO', 'csv', 'LOG FILE CREATED:'+str(self.logfile_name)])
    ...
    def delete_old_log_files(self):
        ...
        for i in range(len(all_log_file_list) - self.log_rotate_count):
            ...
            self.datalog('INFO', 'FILE_DELETED', self.log_dir+self.line_name + '_' + all_log_file_list[i] + '_logfile.csv')
    ...
    def generate_token(self):
        ...
        if x.status_code == 200:
            self.datalog('INFO', 'TOKEN', 'TOKEN_GENERATED')
            return x.json()
                    ...
                    try:
                        ...
                        if len(csv_list) > 0:
                            ...
                            if self.send_file(max_file_path, table) == 200:
                                ...
                                print("||file_sent:" ,table + '_' + str(formatted_timestamp) + '.csv', "||sent at:",
                                      datetime.now() , "|" , self.line_name)
                        ...
                        self.datalog(table, 'FILE_DELETED', e)
                        print(e)
                else:
                    self.datalog('INFO', 'NOT_FOUND', folder_path)
